...
u8 random,temp=0;
random=0;//RNG_Get_RandomRange(0,100);
#define RATE_BIG 60
if(random>RATE_BIG)
...
//state_change 
	...
	{//-------------------------------------------------起飞
		case SG_LOW_CHECK://low thr check
			fly_cover_cnt=cnt_shoot=over_time=cnt_back_home=0;tar_need_to_check_odroid[2]=66;
		  tar_cnt=0;
		  for(i=0;i<19;i++)
		    tar_buf[i]=66;
			...
			else if(mode.auto_fly_up==0&&pwmin.sel_in==1&&ALT_POS_SONAR2>MINE_LAND_HIGH&&fabs(Pitch)<15&&fabs(Roll)<15){
						...
						{state=SU_TO_START_POS;
							...
							//--- out_timer_random
							   tar_need_to_check_odroid[2]=2;
								 for(j=0;j<500;j++){		
									 int tar_temp=	0;//RNG_Get_RandomRange(0,9);
									 u8 flag1=0;
											for(i=0;i<19;i++){
